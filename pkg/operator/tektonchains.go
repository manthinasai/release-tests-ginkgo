...
// The format and taskrunStorage parameters are applied to BOTH TaskRun and PipelineRun
// configurations to ensure consistent signing behavior across both resource types.
...
func UpdateTektonConfigForChains(format, taskrunStorage, ociStorage, transparency, repository string, encodingFormat ...string) {
	...
	patchData := fmt.Sprintf(
		`{"spec":{"chain":{"options":{"disabled":false},`+
			...
			`"artifacts.pipelinerun.format":"%s",`+
			`"artifacts.pipelinerun.storage":"%s",`+
			`"artifacts.oci.storage":"%s",`+
			`"transparency.enabled":"%s"`,
		format, taskrunStorage, format, taskrunStorage, ociStorage, transparency)
...
// introduced by tektoncd/chains#1691. storage.oci.repository is cleared. Both TaskRun
// and PipelineRun configurations are reset to ensure a clean state for subsequent tests.
